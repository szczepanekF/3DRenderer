...
void BoardActionsHandler::onMousePressAction(sf::RenderWindow &window) {
    if (!promotionChoice) {
        sf::Vector2i pos = sf::Mouse::getPosition(window);
        int x = pos.x;
        int y = pos.y;
...
void BoardActionsHandler::onMouseReleaseAction(sf::RenderWindow &window) {
    sf::Vector2i pos = sf::Mouse::getPosition(window);
    int x = pos.x;
    int y = pos.y;
    if (promotionChoice) {
...
void BoardActionsHandler::onKeyboardPressAction(sf::Event &event) {
    if (event.key.code == sf::Keyboard::Right) {
        if (event.key.control) {
            chessGame.forwardAllMoves();
        } else {
            chessGame.forwardOneMove();
        }
    } else if (event.key.code == sf::Keyboard::Left) {
        if (event.key.control) {
            chessGame.rewindAllMoves();
        } else {
            chessGame.rewindLastMove();
        }
